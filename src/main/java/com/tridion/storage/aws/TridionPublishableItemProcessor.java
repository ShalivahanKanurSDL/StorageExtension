 ...
 *
 * */
public class TridionPublishableItemProcessor extends TridionCPProcessor
{
	...
	private void process() throws StorageException
	{
		try
		{
			ComponentPresentationIndexData data = this.getSearchDataDirectives();
			if (data == null)
			{
	...
	private ComponentPresentationIndexData getSearchDataDirectives() throws SAXException, IOException, ParserConfigurationException
	{
		...
		while (m.find())
		{
			if (m.groupCount() == 1)
			{
				...
				if (!Utils.StringIsNullOrEmpty(searchDataXml))
				{
					...
					return registerSearchDataFields(d,searchDataXml);
				}
	...
	private ComponentPresentationIndexData registerSearchDataFields(Document searchDataDocument,String searchDataXml)
	{
		ComponentPresentationIndexData data = new ComponentPresentationIndexData(this.action, this.indexType, this.publicationId, this.storageId,searchDataXml);
